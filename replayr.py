...
Copyright(c) 2011 Thomas Hikade <thomashikade@gmail.com>
MIT Licensed
"""
...
# This pattern matches the Apache combined format
# LogFormat "%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\"" combined

LOGLINE_RE = re.compile(\
    r'(?P<ip>\S+) (\S+) (\S+) \[(?P<time>.*?)\] "(?P<method>\S+) (?P<path>\S+) (?P<version>\S+)" (?P<code>\S+) (?P<size>\S+)')
...
def main(f, host, exclude):
    ...
    for line in loglines:
        ...
        if r is None:
            ...
            continue
        if exclude and pattern.search(r['path']):
            print "filtered: " + repr(line)
            continue

        if not is_replayable_request(r):
            print "cannot replay: %s %s  [status: %s]" % (r['method'], r['path'],r['code'])
            continue
